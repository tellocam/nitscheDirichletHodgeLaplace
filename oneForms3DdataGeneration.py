...
ns = [3, 4, 6, 8]                               # vertices per direction of geometry, translates to 1/n = h
...
for n in ns:
    ...
    for order in orders:
        u, p, ndofs, eX = nhl.hodgeLaplaceOneForms3D(mesh, g, order, Cw)
        results.append({'h': 1/n, 'ndofs':ndofs, 'order':order, 'eX':eX})

df_oneForms2D = pd.DataFrame(results)
df_oneForms2D.to_csv("data/oneForms3Ddata.csv", index=False)
